...
router.post("/registration",async(req,res)=>{
    ...
    await sendEmail(registered.email, "Email verification from PlacementDiaries", message);
...
router.get("/user/verify/:id/:token", async (req, res) => {
    try {
     ...
     await registrationData.updateOne({ _id: user._id}, {$set: {verified: true}});
      ...
      res.send(`<h3>Email verified successfully. To login click the link below</h3> <a href="https://placementdiaries.herokuapp.com/Login">login</a>`);
    } catch (error) {
